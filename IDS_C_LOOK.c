...
Name:		IDS_C_LOOK.c
Author:		Hu Jingmei
Function:	The source code for Disk Servers in Intelligent disk-storage system. Use C_LOOK scheduler to calculate the track-to-track delay time.
*/
...
int main(int argc, char *argv[])
{
	...
	int c = 0, s = 0, l = 0;
	int *cylinders_n = new int [cnt_n];
	int *c_look_q = new int [counter];
	int c_look_time = 0;
	int k = 0, cur_pos = 0, old_pos = 0;
	...
	{	
		...
		//generate the C-LOOK queue
		qsort(0, cnt_n-1, cylinders_n);
		for(cur_pos = 0; cur_pos < cnt_n; cur_pos++) if(cylinders_n[cur_pos] >= old_pos) break;
		c_look_time += Ttime*abs(cylinders_n[cnt_n-1] - old_pos);	//first time, the point is 0, direction : 0 --> cur_pos
		if(cur_pos > 0)
		{
			c_look_time += Ttime*abs(cylinders_n[cnt_n-1] - cylinders_n[0]);//last to first
			c_look_time += Ttime*abs(cylinders_n[cur_pos-1] - cylinders_n[0]);
		}
		for(int i = cur_pos; i < cnt_n; i++) c_look_q[k++] = cylinders_n[i];
		for(int i = 0; i < cur_pos; i++) c_look_q[k++] = cylinders_n[i];
		old_pos = c_look_q[k-1];
	}
	...
	if(counter%cnt_n != 0)
	{
		...
		//generate the C-LOOK queue
		qsort(0, counter%cnt_n-1, cylinders_n);
		for(cur_pos = 0; cur_pos < counter%cnt_n; cur_pos++) if(cylinders_n[cur_pos] >= old_pos) break;
		c_look_time += Ttime*abs(cylinders_n[counter%cnt_n-1] - old_pos);	//first time, the point is 0, direction : 0 --> cur_pos
		if(cur_pos > 0)
		{
			c_look_time += Ttime*abs(cylinders_n[counter%cnt_n-1] - cylinders_n[0]);//last to first
			c_look_time += Ttime*abs(cylinders_n[cur_pos-1] - cylinders_n[0]);
		}
		for(int i = cur_pos; i < counter%cnt_n; i++) c_look_q[k++] = cylinders_n[i];
		for(int i = 0; i < cur_pos; i++) c_look_q[k++] = cylinders_n[i];
	}	
	exist: ;	
	usleep(Ttime*abs(c_look_time));		
	printf("C-LOOK queue: ");		
	for(int i = 0; i < counter; i++) cout << c_look_q[i] << " "; cout << endl;
	printf("C-LOOK time: %d\n", c_look_time);
