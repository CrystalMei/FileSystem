...
Name:		IDS_SSTF.c
Author:		Hu Jingmei
Function:	The source code for Disk Servers in Intelligent disk-storage system. Use SSTF scheduler to calculate the track-to-track delay time.
*/
...
int main(int argc, char *argv[])
{
	...
	int *sstf_q = new int [counter];
	int *sstf_found = new int [cnt_n];
	int *sstf_tmp = new int [cnt_n];
	for(int i = 0; i < cnt_n; i++) sstf_found[i] = 0;
	int k = 0;
	int cur_pos = 0;
	int sstf_time = 0;
	...
	{	
		...
		//generate the SSTF queue
		for(int i = 0; i < cnt_n; i++)
		{		
			for(int j = 0; j < cnt_n; j++) sstf_tmp[j] = abs(cylinders_n[j]-cur_pos);
			int tmp = findmin(cnt_n, cur_pos, sstf_tmp, sstf_found);
			sstf_q[k++] = cylinders_n[tmp];
			sstf_found[tmp] = 1;
			sstf_time += Ttime*abs(cylinders_n[tmp]-cur_pos);
			cur_pos = cylinders_n[tmp]; 
		}
	}
	...
	if(counter%cnt_n != 0)
	{
		...
		for(int i = 0; i < counter%cnt_n; i++)
		{		
			for(int j = 0; j < counter%cnt_n; j++) sstf_tmp[j] = abs(cylinders_n[j]-cur_pos);
			int tmp = findmin(counter%cnt_n, cur_pos, sstf_tmp, sstf_found);
			sstf_q[k++] = cylinders_n[tmp];
			sstf_found[tmp] = 1;
			sstf_time += Ttime*abs(cylinders_n[tmp]-cur_pos);
			cur_pos = cylinders_n[tmp]; 
	...
	usleep(Ttime*abs(sstf_time));			
	printf("SSTF queue: ");	
	for(int i = 0; i < counter; i++) cout << sstf_q[i] << " "; cout << endl;
	printf("SSTF time: %d\n", sstf_time);
